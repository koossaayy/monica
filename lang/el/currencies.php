<?php

return [
    'AED' => 'Ντιρχάμ Ηνωμένων Αραβικών Εμιράτων',
    'AFA' => 'Αφγανί Αφγανιστάν',
    'ALL' => 'Λεκ Αλβανίας',
    'AMD' => 'Ντραμ Αρμενίας',
    'ANG' => 'Γκίλντερ Ολλανδικών Αντιλλών',
    'AOA' => 'Κουάνζα Αγκόλας',
    'ARS' => 'Πέσο Αργεντινής',
    'AUD' => 'Δολάριο Αυστραλίας',
    'AWG' => 'Φλορίνι Αρούμπας',
    'AZN' => 'Μανάτ Αζερμπαϊτζάν',
    'BAM' => 'Βοσνία',
    'BBD' => 'Δολάριο Μπαρμπάντος',
    'BDT' => 'Τάκα Μπανγκλαντές',
    'BEF' => 'Φράγκο Βελγίου',
    'BGN' => 'Λεβ Βουλγαρίας',
    'BHD' => 'Δηνάριο Μπαχρέιν',
    'BIF' => 'Φράγκο Μπουρούντι',
    'BMD' => 'Δολάριο Βερμούδων',
    'BND' => 'Δολάριο Μπρουνέι',
    'BOB' => 'Μπολιβιάνο Βολιβίας',
    'BRL' => 'Ρεάλ Βραζιλίας',
    'BSD' => 'Δολάριο Μπαχαμών',
    'BTC' => 'Bitcoin',
    'BTN' => 'Νγκούλτρουμ Μπουτάν',
    'BWP' => 'Πούλα Μποτσουάνας',
    'BYR' => 'Λευκορωσικό ρούβλι',
    'BZD' => 'Δολάριο Μπελίζ',
    'CAD' => 'Καναδικό δολάριο',
    'CDF' => 'Φράγκο Κονγκό',
    'CHF' => 'Ελβετικό φράγκο',
    'CLP' => 'Χιλιανό πέσο',
    'CNY' => 'Κινεζικό γουάν',
    'COP' => 'Κολομβιανό πέσο',
    'CRC' => 'Κόστα Ρικανικό κολόν',
    'CUC' => 'Κουβανικό μετατρέψιμο πέσο',
    'CVE' => 'Σκούδο Πράσινου Ακρωτηρίου',
    'CZK' => 'Κορόνα Τσεχικής Δημοκρατίας',
    'DEM' => 'Γερμανικό μάρκο',
    'DJF' => 'Φράγκο Τζιμπουτί',
    'DKK' => 'Δανική κορόνα',
    'DOP' => 'Δομινικανό πέσο',
    'DZD' => 'Αλγερινό δηνάριο',
    'EEK' => 'Εσθονική κορόνα',
    'EGP' => 'Αιγυπτιακή λίρα',
    'ERN' => 'Νάκφα Ερυθραίας',
    'ETB' => 'Αιθιοπικό μπιρ',
    'EUR' => 'Ευρώ',
    'FJD' => 'Δολάριο Φίτζι',
    'FKP' => 'Λίρα Νήσων Φόκλαντ',
    'GBP' => 'Βρετανική λίρα στερλίνα',
    'GEL' => 'Γεωργιανό λάρι',
    'GHS' => 'Γκανέζικο σέντι',
    'GIP' => 'Λίρα Γιβραλτάρ',
    'GMD' => 'Νταλάσι Γκάμπιας',
    'GNF' => 'Φράγκο Γουινέας',
    'GRD' => 'Ελληνική δραχμή',
    'GTQ' => 'Κετσάλ Γουατεμάλας',
    'GYD' => 'Δολάριο Γουιάνας',
    'HKD' => 'Δολάριο Χονγκ Κονγκ',
    'HNL' => 'Λεμπίρα Ονδούρας',
    'HRK' => 'Κροατική κούνα',
    'HTG' => 'Γκουρντ Αϊτής',
    'HUF' => 'Ουγγρικό φιορίνι',
    'IDR' => 'Ινδονησιακή ρουπία',
    'ILS' => 'Νέο ισραηλινό σέκελ',
    'INR' => 'Ινδική ρουπία',
    'IQD' => 'Ιρακινό δηνάριο',
    'IRR' => 'Ιρανικό ριάλ',
    'ISK' => 'Ισλανδική κορόνα',
    'ITL' => 'Ιταλική λιρέτα',
    'JMD' => 'Δολάριο Τζαμάικας',
    'JOD' => 'Ιορδανικό δηνάριο',
    'JPY' => 'Ιαπωνικό γεν',
    'KES' => 'Σελίνι Κένυας',
    'KGS' => 'Σομ Κιργιζίας',
    'KHR' => 'Ριέλ Καμπότζης',
    'KMF' => 'Φράγκο Κομορών',
    'KPW' => 'Γουόν Βόρειας Κορέας',
    'KRW' => 'Γουόν Νότιας Κορέας',
    'KWD' => 'Δηνάριο Κουβέιτ',
    'KYD' => 'Δολάριο Νήσων Κέιμαν',
    'KZT' => 'Τένγκε Καζακστάν',
    'LAK' => 'Κιπ Λάος',
    'LBP' => 'Λίρα Λιβάνου',
    'LKR' => 'Ρουπία Σρι Λάνκα',
    'LRD' => 'Δολάριο Λιβερίας',
    'LSL' => 'Λότι Λεσότο',
    'LTL' => 'Λίτας Λιθουανίας',
    'LVL' => 'Λατς Λετονίας',
    'LYD' => 'Δηνάριο Λιβύης',
    'MAD' => 'Ντιρχάμ Μαρόκου',
    'MDL' => 'Λέου Μολδαβίας',
    'MGA' => 'Αριάρι Μαδαγασκάρης',
    'MKD' => 'Δηνάριο Βόρειας Μακεδονίας',
    'MMK' => 'Κιάτ Μιανμάρ',
    'MNT' => 'Τουγκρίκ Μογγολίας',
    'MOP' => 'Πατάκα Μακάου',
    'MRO' => 'Ουγκίγια Μαυριτανίας',
    'MUR' => 'Ρουπία Μαυρικίου',
    'MVR' => 'Ρουφίγια Μαλδίβων',
    'MWK' => 'Κουάτσα Μαλάουι',
    'MXN' => 'Πέσο Μεξικού',
    'MYR' => 'Ρινγκίτ Μαλαισίας',
    'MZM' => 'Μετικάλ Μοζαμβίκης',
    'NAD' => 'Δολάριο Ναμίμπια',
    'NGN' => 'Νάιρα Νιγηρίας',
    'NIO' => 'Κόρδοβα Νικαράγουας',
    'NOK' => 'Κορόνα Νορβηγίας',
    'NPR' => 'Ρουπία Νεπάλ',
    'NZD' => 'Δολάριο Νέας Ζηλανδίας',
    'OMR' => 'Ριάλ Ομάν',
    'PAB' => 'Μπαλμπόα Παναμά',
    'PEN' => 'Νουέβο σολ Περού',
    'PGK' => 'Κίνα Παπούας Νέας Γουινέας',
    'PHP' => 'Πέσο Φιλιππίνων',
    'PKR' => 'Ρουπία Πακιστάν',
    'PLN' => 'Ζλότι Πολωνίας',
    'PYG' => 'Γκουαρανί Παραγουάης',
    'QAR' => 'Ριάλ Κατάρ',
    'RON' => 'Λέου Ρουμανίας',
    'RSD' => 'Δηνάριο Σερβίας',
    'RUB' => 'Ρούβλι Ρωσίας',
    'RWF' => 'Φράγκο Ρουάντας',
    'SAR' => 'Ριάλ Σαουδικής Αραβίας',
    'SBD' => 'Δολάριο Νήσων Σολομώντα',
    'SCR' => 'Ρουπία Σεϋχελλών',
    'SDG' => 'Λίρα Σουδάν',
    'SEK' => 'Κορόνα Σουηδίας',
    'SGD' => 'Δολάριο Σιγκαπούρης',
    'SHP' => 'Λίρα Αγίας Ελένης',
    'SKK' => 'Κορόνα Σλοβακίας',
    'SLL' => 'Λεόνε Σιέρα Λεόνε',
    'SOS' => 'Σελίνι Σομαλίας',
    'SRD' => 'Δολάριο Σουρινάμ',
    'STD' => 'Ντόμπρα Σάο Τομέ και Πρίνσιπε',
    'SVC' => 'Κολόν Ελ Σαλβαδόρ',
    'SYP' => 'Λίρα Συρίας',
    'SZL' => 'Λιλανγκένι Εσουατίνι',
    'THB' => 'Μπατ Ταϊλάνδης',
    'TJS' => 'Σομόνι Τατζικιστάν',
    'TMT' => 'Μανάτ Τουρκμενιστάν',
    'TND' => 'Δηνάριο Τυνησίας',
    'TOP' => 'Παάνγκα Τόνγκα',
    'TRY' => 'Λίρα Τουρκίας',
    'TTD' => 'Δολάριο Τρινιντάντ και Τομπάγκο',
    'TWD' => 'Νέο δολάριο Ταϊβάν',
    'TZS' => 'Σελίνι Τανζανίας',
    'UAH' => 'Χρίβνια Ουκρανίας',
    'UGX' => 'Σελίνι Ουγκάντας',
    'USD' => 'Δολάριο ΗΠΑ',
    'UYU' => 'Πέσο Ουρουγουάης',
    'UZS' => 'Σομ Ουζμπεκιστάν',
    'VEF' => 'Μπολίβαρ Βενεζουέλας',
    'VND' => 'Ντονγκ Βιετνάμ',
    'VUV' => 'Βατού Βανουάτου',
    'WST' => 'Τάλα Σαμόα',
    'XAF' => 'Φράγκο CFA BEAC',
    'XCD' => 'Δολάριο Ανατολικής Καραϊβικής',
    'XDR' => 'Ειδικά Τραβηκτικά Δικαιώματα',
    'XOF' => 'Φράγκο CFA BCEAO',
    'XPF' => 'Φράγκο CFP',
    'YER' => 'Ριάλ Υεμένης',
    'ZAR' => 'Ραντ Νότιας Αφρικής',
    'ZMK' => 'Κουάτσα Ζάμπιας',
];
